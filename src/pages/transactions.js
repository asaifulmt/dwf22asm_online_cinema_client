...
export default function Transactions() {
  ...
  return (
    <Container className="mt-5">
      ...
      {msg.text && (
        <Alert variant={msg.error ? "danger" : "primary"}>{msg.text}</Alert>
      )}
        ...
        <tbody>
          {transactions.map((tr, idx) => (
            <tr key={tr.id}>
              ...
              <td>
                <a
                  href={`https://online-cinema-api.herokuapp.com/uploads/${tr.transferProof}`}
                >
                  {tr.transferProof}
                </a>
              </td>
              ...
              <td>
                <DropdownButton title="Action">
                  <Dropdown.Item
                    onClick={() => updateStatus(tr.id, "approved")}
                  >
                    Approve
                  </Dropdown.Item>
                  <Dropdown.Item onClick={() => updateStatus(tr.id, "pending")}>
                    Pending
                  </Dropdown.Item>
                  <Dropdown.Item
                    onClick={() => updateStatus(tr.id, "canceled")}
                  >
                    Cancel
                  </Dropdown.Item>
                </DropdownButton>
